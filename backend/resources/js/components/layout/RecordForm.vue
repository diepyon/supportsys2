<template>
  <div style="padding-top: 16px">
    <v-form ref="test_form" v-model="valid" lazy-validation>
      ...
      <v-text-field v-model="value.dealer" label="販売店" solo></v-text-field>
      <v-row justify="start">
        <v-col cols="5">
          <v-text-field
            v-model="value.questioner"
            label="問い合わせ者"
            solo
          ></v-text-field>
        </v-col>
      ...
      <v-container fluid>
        <v-radio-group v-model="value.remote" row>
          ...
          <v-col cols="5">
            <v-text-field
              v-if="value.remote == 'なし'"
              v-model="value.authorizer"
              label="承認者"
              disabled
              solo
            >
